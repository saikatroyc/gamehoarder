...
def unwiki(text, quiet = True):
    ...
    regexes = [('', r'[V|v]gy\s*\|\|?(\d{4})(?:\|\d{4})?'),
               ...
               ('', r'[C|c]ollapsible[^\|\{\}]*\|\s*(?:title\s*=\s*)?([^\|]*?)\|?(?:\s*titlestyle[^\|]*?\|)?(.*?)'),
               ('', r'[F|f]lat[ ]?list\|([^\{\}]+)'),
               ('', r'[U|u]bl\|((?:[^\|\{\}]+\|{0,3})+)'),
               ('', r'[U|u]nbulleted [L|l]ist([^\{\}]*?)'),
               ('', r'[H|h]list\s*\|([^\{\}]+?)'),
               ('', r'([C|c]heck mark)\|[^\{\}]*?'),
               ('', r'([C|c]ross)\|[^\{\}]*?'),
               ('', r'[P|p]lain.?list\s*\|([^\{\}]*?)\s*'),
...
def year(text):
    ...
    date1 = re.search(r"([A-Z|a-z|\.]+) (\d+), (\d{4})", text)
    if (date1):
...
def mine_wiki_info(name, sub_url, depth = 2):
    ...
    if len(html) < 500 and "Infobox" not in html and "infobox" not in html and \
       "#REDIRECT" in html:
        (link, _) = find_wiki_url(html[html.find("#REDIRECT") + 8 :])
        return mine_wiki_info(name, link, depth - 1) if depth > 0 else {}
    ...
    for line, next in zip(lines, lines[1:]):
        ...
        if "infobox" in line.lower() and not infobox and \
           ("vg" in line.lower() or "game" in line.lower()):
            infobox = True
...
def mine_wiki_page(title, url, f = None, games = {}, skip_to_game = None, infobox = False):
    ...
    def readURL(thread_id):
        while (True):
            try:
                ...
                (names, new_games) = mine_wiki_info(name, link)
...
def mine_wiki(f = None, games = {}, skip_to = (None, None), end_at = None, infobox = False):
    ...
    for pf in pfs:
        ...
        if not skip_to_plat:
            ...
            # otherwise, use wiki api to find game info, by reading wiki markup source
                ...
                games.update(mine_wiki_page(title, api_base + pf.attrib['href'][6:], f, games, skip_to_game, infobox))
